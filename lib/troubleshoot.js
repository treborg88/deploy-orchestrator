...
const TROUBLESHOOT_DB = [
  ...
  {
    patterns: [' 502 ', 'status of 502', 'favicon.ico', 'no live upstreams', 'connect() failed', 'bad gateway'],
    title: 'Proxy Upstream 502 (Frontend/Backend unreachable)',
    fixes: [
      'Nginx is running, but it cannot reach frontend or backend upstream containers.',
      'Use the new VM diagnostics pull in the orchestrator to capture docker compose status and proxy/frontend/backend logs.',
      'Check if containers are up: cd ~/TechStore && docker compose ps',
      'Check proxy logs for upstream errors: docker compose logs --tail 120 proxy',
      'Recreate and restart services: docker compose up -d --force-recreate backend frontend proxy && docker compose restart proxy',
      'Validate local HTTPS routes on VM: curl -kI https://127.0.0.1/api/products and curl -kI https://127.0.0.1/favicon.ico'
    ]
  },
  {
