  ...
  await new Promise((resolve, reject) => {
    const args = [
      ...
      [
        dumpCmd,
        `test -s ${remoteFile}`,
        `grep -q "landingPageConfig" ${remoteFile} && echo LANDING_CONFIG_IN_DUMP || echo LANDING_CONFIG_MISSING`,
        `wc -c < ${remoteFile}`,
        'echo DUMP_OK'
      ].join(' && ')
    ];
    ...
    proc.on('close', (code) => {
      if (code === 0 && stdout.includes('DUMP_OK')) {
        if (stdout.includes('LANDING_CONFIG_MISSING')) {
          return reject(new Error('Backup SQL generado, pero NO contiene landingPageConfig. Verifica que los cambios estén guardados en DB antes de respaldar.'));
        }
        resolve();
      } else {
...
export async function restoreDatabase(host, user, keyPath, deployMode, localFile, broadcast) {
  ...
  if (deployMode === 'docker') {
    ...
    restoreCmd = [
      ...
      `docker exec -i techstore-db psql -U techstore -d techstore -c "INSERT INTO app_settings (id, value) VALUES ('landingPageConfig', '{\"enabled\":false}') ON CONFLICT (id) DO NOTHING;"`,
      `echo RESTORE_OK`
    ].join(' && ');
  } else {
    ...
    restoreCmd = [
      ...
      `psql "$DB_URL" -c "INSERT INTO app_settings (id, value) VALUES ('landingPageConfig', '{\"enabled\":false}') ON CONFLICT (id) DO NOTHING;"`,
      'echo RESTORE_OK'
    ].join(' && ');
  }
  ...
  // Step 2.1: Verify landing settings after restore (critical for landing page persistence)
  broadcast('log', { text: '$ Verifying landingPageConfig after restore...' });
  await new Promise((resolve, reject) => {
    const verifyCmd = deployMode === 'docker'
      ? `docker exec -i techstore-db psql -U techstore -d techstore -tAc "SELECT COALESCE(length(value),0) FROM app_settings WHERE id='landingPageConfig';"`
      : [
          'source /home/ubuntu/TechStore/backend/.env 2>/dev/null || true',
          'DB_URL="${DATABASE_URL:-postgresql://techstore:techstore@localhost:5432/techstore}"',
          `psql "$DB_URL" -tAc "SELECT COALESCE(length(value),0) FROM app_settings WHERE id='landingPageConfig';"`
        ].join(' && ');
    ...
    proc.on('close', (code) => {
      if (code !== 0) {
        return reject(new Error(`Post-restore verification failed (exit ${code}): ${stderr || stdout}`));
      }
      const landingLen = parseInt((stdout || '').trim(), 10) || 0;
      if (landingLen <= 0) {
        return reject(new Error('Restore completed but landingPageConfig is missing/empty in app_settings.'));
      }
      broadcast('log', { text: `✅ landingPageConfig restored (${landingLen} chars)` });
      resolve();
    });
    proc.on('error', (err) => reject(new Error(`SSH failed: ${err.message}`)));
  });
...
/**
 * Force Deploy (Docker) — discard tracked local changes, sync to origin/main, rebuild and restart.
 * Keeps Docker volumes intact (no data reset).
 */
export async function forceDeployDocker(host, user, keyPath, broadcast) {
  return runQuickSSH(host, user, keyPath,
    'cd /home/ubuntu/TechStore && git fetch origin && git reset --hard origin/main && sudo docker compose build --no-cache && sudo docker compose up -d && echo "FORCE_DEPLOY_OK" && sudo docker compose ps',
    broadcast, 'Force Deploy (Docker)'
  );
}
...
// =============================================================================
// Deployment Diagnostics — Pull actual VM state for failed deployments
// =============================================================================

/**
 * Collect server diagnostics through SSH in a single run.
 * Useful when deploy/redeploy fails and we need the real VM state.
 *
 * @param {string} host - Server IP
 * @param {string} user - SSH username
 * @param {string} keyPath - Path to SSH key
 * @param {Function} broadcast - WebSocket broadcast function
 * @returns {Promise<{success: boolean, timestamp: string, parsed: Object, raw: string}>}
 */
export async function collectServerDiagnostics(host, user, keyPath, broadcast) {
  const diagCmd = [
    'set +e',
    'echo "##TS##" && date -Iseconds',
    'echo "##PWD##" && pwd',
    'echo "##TECHSTORE_LS##" && ls -la /home/ubuntu/TechStore 2>&1',
    'echo "##DOCKER_PS##" && (cd /home/ubuntu/TechStore && sudo docker compose ps 2>&1 || echo "docker-compose-unavailable")',
    'echo "##PROXY_LOGS##" && (cd /home/ubuntu/TechStore && sudo docker compose logs --tail 120 proxy 2>&1 || true)',
    'echo "##FRONTEND_LOGS##" && (cd /home/ubuntu/TechStore && sudo docker compose logs --tail 80 frontend 2>&1 || true)',
    'echo "##BACKEND_LOGS##" && (cd /home/ubuntu/TechStore && sudo docker compose logs --tail 80 backend 2>&1 || true)',
    'echo "##NGINX_STATUS##" && (sudo systemctl status nginx --no-pager -l 2>&1 | head -30 || echo "nginx-systemd-not-available")',
    'echo "##HTTP_LOCAL##" && (curl -sS -I --max-time 8 http://127.0.0.1/ 2>&1 || true)',
    'echo "##HTTPS_LOCAL##" && (curl -k -sS -I --max-time 8 https://127.0.0.1/ 2>&1 || true)',
    'echo "##HTTPS_API_LOCAL##" && (curl -k -sS -I --max-time 8 https://127.0.0.1/api/products 2>&1 || true)',
    'echo "##HTTPS_FAVICON_LOCAL##" && (curl -k -sS -I --max-time 8 https://127.0.0.1/favicon.ico 2>&1 || true)',
    'echo "##END##"'
  ].join(' && ');

  const result = await runQuickSSH(
    host,
    user,
    keyPath,
    diagCmd,
  ...
  const parsed = {
    timestamp: parseSection('TS'),
    pwd: parseSection('PWD'),
    techstoreLs: parseSection('TECHSTORE_LS'),
    dockerPs: parseSection('DOCKER_PS'),
    proxyLogs: parseSection('PROXY_LOGS'),
    frontendLogs: parseSection('FRONTEND_LOGS'),
    backendLogs: parseSection('BACKEND_LOGS'),
    nginxStatus: parseSection('NGINX_STATUS'),
    httpLocal: parseSection('HTTP_LOCAL'),
    httpsLocal: parseSection('HTTPS_LOCAL'),
    httpsApiLocal: parseSection('HTTPS_API_LOCAL'),
    httpsFaviconLocal: parseSection('HTTPS_FAVICON_LOCAL')
  };
